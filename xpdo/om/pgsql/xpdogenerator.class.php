...
/**
 * PostgreSQL classes for generating xPDOObject classes and maps from an xPDO schema.
 *
 * @package xpdo
 * @subpackage om.mysql
 */
 ...
 *
 * A PostgreSQL-specific extension to an {@link xPDOManager} instance that can
...
class xPDOGenerator_pgsql extends xPDOGenerator {
    ...
    public function writeSchema($schemaFile, $package = '', $baseClass = '', $tablePrefix = '', $restrictPrefix = false) {
        ...
        //read list of tables
        $tablesStmt = $this->manager->xpdo->query("SELECT table_name FROM information_schema.tables WHERE table_schema = 'public'");
        ...
        foreach ($tables as $table) {
            ...
            $xmlFields = array();
            $xmlIndices = array();
            if (!$tableName = $this->getTableName($dbTableName, $tablePrefix, $restrictPrefix)) {
                continue;
            }
            $class = $this->getClassName($tableName);
            $extends = $baseClass;
            $fieldsStmt = $this->manager->xpdo->query("SELECT column_name, data_type, udt_name, numeric_precision, character_maximum_length, is_nullable, column_default FROM information_schema.columns WHERE table_name = '$dbTableName' ORDER BY ordinal_position");
            if ($fieldsStmt) {
...
//                        }
                        $precisionAttribute = empty($field['character_maximum_length']) ? '' : ' precision="' . $field['character_maximum_length'] . '"';
                        $phpType = $this->manager->xpdo->driver->getPhpType($field['data_type']);
                        $nullAttribute = ' null="' . (($Null === 'NO') ? 'false' : 'true') . '"';
//                        $Key = $this->getIndex($Key);
                        $dataTypeValue = $field['data_type'];
                        $defaultValue = $field['column_default'];
                        if ($phpType === 'integer' && preg_match('/^nextval\(/', $field['column_default'])) {
                            // This is a sequence (equ. to auto_increment)
                            if ($baseClass === 'xPDOObject' && $field['column_name'] === 'id') {
                                $extends = 'xPDOSimpleObject';
                                continue;
                            } else {
                                $dataTypeValue = $field['numeric_precision'] == 32 ? 'serial' : 'bigserial';
                                $defaultValue = '';
                            }
                        }
                        
                        $defaultAttribute = $this->getDefault($defaultValue);                
                        $xmlFields[] = "\t\t<field key=\"{$field['column_name']}\" dbtype=\"{$dataTypeValue}\"{$precisionAttribute} phptype=\"{$phpType}\"{$nullAttribute}{$defaultAttribute} />";
                        HALLÅ HGALLÅ KOLLA PRIMARY KEY OCH UNIQUE DEFF!!om 2och 
                    }
                } else {
